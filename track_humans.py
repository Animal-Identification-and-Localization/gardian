...
def main():
    ...
    def user_callback(input_tensor, src_size, inference_box):
      ...
      for idx in range(0,len(objs)):
        ...
        elif objs[idx].id == 1 and area<area_target:
          target = objs[idx]
          area_target = area
      ...
      if target is not None:
        ...
        width   = target.bbox.xmax-target.bbox.xmin
        height  = target.bbox.ymax-target.bbox.ymin

        dx = int(int((dx - inference_size[0]/2)/5)*4)*int(abs(dx1)>=width/3)
        if dx<0: dx = max(dx, -200)
        if dx>0: dx = min(dx, 200)

        dy = int(int((dy - inference_size[1]/2))/5)*int(abs(dy1)>=height/3)
        ...
        if not no_spi: 
          try: 
            ...
            # start_time = time.monotonic()
            # if dy-prev_dy < 
            send_dx_dy(-1*dx, -1*int(dy), i2c, False)
